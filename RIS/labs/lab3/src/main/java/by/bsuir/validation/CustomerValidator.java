...
public class CustomerValidator implements EntityValidator {
    ...
    public ValidationResult validate(Object objectToValidate) {
        ...
        BiPredicate<String, String> text = (pattern, valueToCheck) -> !Pattern.matches(pattern, valueToCheck);
        if (objectToValidate instanceof Customer) {
            Customer customer = (Customer) objectToValidate;
            if (text.test(TEXT_PATTERN, customer.getName())) {
            ...
            if (text.test(ADDRESS_PATTERN, customer.getMainAddress())) {
                ValidationResult.ValidationError error = new ValidationResult.ValidationError();
            ...
            if (!customer.getAdditionalAddress().equals("") && text.test(ADDRESS_PATTERN, customer.getAdditionalAddress())) {
                ValidationResult.ValidationError error = new ValidationResult.ValidationError();
